...
def _svg(w: int, h: int, body: str, title: str, desc: str = "") -> str:
    # width/height give the file an intrinsic size. Loaded through <img> an
    # SVG without them falls back to a 300x150 default box.
    return (f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {w} {h}" '
            f'width="{w}" height="{h}" '
            f'role="img" aria-labelledby="t d" font-family="system-ui, sans-serif">'
...
def lateralisation_figure() -> str | None:
    ...
    parts.append(f'<text x="{mid - half}" y="18" font-size="11" fill="var(--fg-mut)">'
                 f'more active drawing with the RIGHT hand</text>')
    parts.append(f'<text x="{mid + 6}" y="18" font-size="11" fill="var(--fg-mut)">'
    ...
    for i, r in enumerate(rows):
        ...
        parts.append(f'<text x="{tx:.1f}" y="{y + 14}" font-size="10" '
                     f'text-anchor="{anchor}" fill="var(--fg-faint)">{tag}</text>')

    parts.append(f'<text x="{lab_w}" y="{h - 16}" font-size="11" fill="var(--fg-mut)">'
